...
talpha = t.ppf(alpha, nu_hat)
pdf = t.pdf(talpha, nu_hat)

C = (nu_hat + talpha**2) / ((nu_hat - 1) * alpha) * pdf
ES_diff = sigma_hat * C
ES_abs  = - (mu_hat - ES_diff)

pd.DataFrame({"ES Absolute":[ES_abs], "ES Diff from Mean":[ES_diff]}).to_csv("out_8.5.csv", index=False)
